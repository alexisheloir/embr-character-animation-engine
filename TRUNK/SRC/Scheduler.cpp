...
void Scheduler::enqueueMotionSegment(MotionSegment *_segment)
{
  ...
  while (segmentIterator != m_motionSegments.end())
  {
    ...
    if (currentSegment->getAbsoluteStartTime() >= _segment->getAbsoluteStartTime() && currentSegment->getAbsoluteStopTime() != 0.0f ) // so that breathing is always the last motion segment
    {
